...
unsigned int nStakeMinAge = 60 * 60 *  8  * 1;	//minimum age for coin age:  1d
...
int64 nChainStartTime = 1463818143;
...
const string strMessageMagic = "LongCoin Signed Message:\n";
...
static int FINAL_POW_HEIGHT = 1208986;
static int FINAL_POW_COUNT = 590000;
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, const CBlockIndex* pindex)
{
     ...
     if (nPoWHeight < 1000)
       nSubsidy = 158000 * COIN; //Premin: 158000000
     else
       nSubsidy = 0 * COIN;
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    nRewardCoinYear = MAX_CLOAK_PROOF_OF_STAKE;
    int64 nSubsidy = nCoinAge * nRewardCoinYear / 365;

    if(nHeight <  YEARLY_POS_BLOCK_COUNT * 1)
      nSubsidy *= 5;
    else if(nHeight <  YEARLY_POS_BLOCK_COUNT * 2)
      nSubsidy *= 4;
    else if(nHeight <  YEARLY_POS_BLOCK_COUNT * 3)
      nSubsidy *= 3;
    else if(nHeight <  YEARLY_POS_BLOCK_COUNT * 4)
      nSubsidy *= 2;
    else if(nHeight >  YEARLY_POS_BLOCK_COUNT * 5)
      nSubsidy = 0;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in LongCoin
    bool fStrictPayToScriptHash = true; // Always active in LongCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "LongCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "The bitcoin price goes to 439.28$ on bitfinex 2016/05/21 16:16 UTC+8, Wahoo!";
        CTransaction txNew;
      ...
      if (true  && (block.GetHash() != hashGenesisBlock))
        ...
        assert(block.hashMerkleRoot == uint256("0xda8fcd2062e6e88ebd55a17bb6fb278b693985bd5cb17e85490cf1b17541d694"));
...
unsigned char pchMessageStart[4] = { 0x74, 0xf5, 0x1c, 0x61 };
